...
export class ParseService {
  ...
  async processAllFiles(event: ParseEvent) {
    ...
    const fileList = await this.s3.listBucketContents(
        `pcc-integration-data-files-${process.env.RUNTIME_ENV}`
      ) || [];

      this.appLogger.log(`Found ${fileList ? fileList.length : 0} files in bucket...`)

    try {

      const allFiles = await this.getAllFiles();

      this.appLogger.log(`Found ${allFiles.length} files already uploaded to the  database...`)
      
      const allUploadedFiles: string[] = allFiles.map((f) => f.sourceFileName);
      
      const parseList = 
        event.Records.length === 0 ? _.difference(fileList, allUploadedFiles) : _.difference(eventFileList,
        allUploadedFiles)
  ...
  async commenceDailyUpload(date: Date) {
    const rules = await this.alertService.getAllRules();
    for (const rule of rules) {
      const daily = await this.alertService.getDailyForRule(rule, new Date(date));
      if (!daily) {
